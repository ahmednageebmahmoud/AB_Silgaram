...
namespace DAL.Service
{
    public class MenuService : IDisposable
    {
        ...
        public object GetControlPanelMenu()
        {
            ...
            List<PagesRole> PagesAllowToCurrentUsere = db.PagesRoles.Where(c => (c.IsCreate || c.IsDelete || c.IsDisplay || c.IsUpdate)
            && c.FkPageRoleTypes_Id == PagesRoleTypeEnumVM.Management
            && CurrentUserRoles.Any(v => v == c.FkRole_Id)
            && c.Page.IsControlPanel).ToList().Select(c => new PagesRole() { FkPage_Id = c.FkPage_Id }).ToList();
